...
%% helper functions
    ...
    function [snips,sptm] = get_spike_snips( sptm,data )
        ...
        maxPk = max(spamp,[],2);
        bad = maxPk>0 | sum( abs( spamp ),2 ) >= artifact*m;
        snips(:,bad,:) = [];
        sptm(bad) = [];      
    ...
    function [snips,sptm,vMin,vMax] = get_aligned_spikes( spikes,sptm )
        ...
        for c = 1:nChan
            upspikes(:,:,c) = csaps( x,spikes(:,:,c)',.3,xx )'; % smooths the interpolant
        end
        upFs = Fs * 4;
