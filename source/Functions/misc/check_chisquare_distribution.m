function [chisqStat,pval] = check_chisquare_distribution( X,df )
    ...
    % compute the test statistic using CDFs, which are 
    % more stable as the PDF may have entries = 0 
    % (since we are working with a PMF)
    chisqStat = sum( (cumsum( O ) - E).^2 ./ E );   
