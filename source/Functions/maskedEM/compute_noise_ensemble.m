...
%       x:
%           n x p feature matrix with (i,j) replaced by noiseDist(j) with P( 1 - mask(i,j) )
%       y:
%           n x p mean expectation matrix of x: mask(i,j) * features(i,j) + (1-mask(i,j)) * mu(j)
%       z:
%           n x p variance expectation matrix of x: mask(i,j) * features(i,j)^2 + (1-mask(i,j)) * (mu(j)^2 + sigma(j)^2)
%       eta:
...
% Replace features(i,j) with noise distribution with P( 1 - mask(i,j) )
noiseDistMat = repmat( model.noiseDist,n,1 );
replaceObs = rand( n,n ) > mask;
features(replaceObs) = noiseDistMat(replaceObs);
model.x = features;
